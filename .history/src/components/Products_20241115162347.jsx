...
const Products = () => {
  ...
  const renderStarRating = () => {
     ...
     products.map((product) =>return setRating(product.rating.rate) , setfullStars(Math.floor(rating)), sethasHalfStar( rating % 1 >= 0.5) )
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex-grow ">
        ...
        <p className="mt-1 text-gray-900 font-semibold">Rating:{rating()} {product.rating.rate} <span className='bg-black !text-white inline-block w-[40px] text-center rounded-full'>{product.rating.count}</span> </p>
      </div>
