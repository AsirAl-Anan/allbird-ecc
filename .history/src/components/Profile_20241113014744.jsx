...
import { EmailAuthProvider, GoogleAuthProvider, updateEmail } from 'firebase/auth/web-extension';
import { EmailAuthProvider, GoogleAuthProvider, updateEmail, reauthenticateWithCredential, reauthenticateWithPopup, sendEmailVerification } from 'firebase/auth';
...
const Profile = () => {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      if(providerId === 'password'){
         ...
         auth.currentUser.updateEmail(newEmail).then(()=>
        {
          sendEmailVerification(auth.currentUser).then(()=>{
            setMessage('Verification email sent to the new email address. Please verify it before we proceed with the update.')
          }).catch((error)=> setMessage('failed to send email', error))
        })
      ...
      setMessage('Failed to update emai: ', error.message)
    }
