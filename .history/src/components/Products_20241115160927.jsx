      ...
      <main className="flex-grow">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-[1500px] mx-auto px-4">
          {products.map((product) => (
            <ProductCard key={product.id} product={product} rating= />
          ))}
        </div>
      </main>
      <Footer />
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex-grow ">
        ...
        <p className="mt-1 text-gray-900 font-semibold">Rating:{renderStarRating()} {product.rating.rate} <span className='bg-black !text-white inline-block w-[40px] text-center rounded-full'>{product.rating.count}</span> </p>
      </div>
