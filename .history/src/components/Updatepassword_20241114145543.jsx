...
const Updatepassword = () => {
 ...
 const {userCredential , user, reauthenticateWithCredential, EmailAuthProvider, updatePassword} = useContext(AuthContext)
...
const [showNewPass, setShowNewPass] = useState(false);
  ...
  console.log(passUpdateRef.current.style)
  useEffect(()=>{
    if(message){
        toast(message)
    }
   ...
   useEffect(()=>{
    if(display){
      passUpdateRef.current.classlist.add('block')
    } else {
      passUpdateRef.current.classlist.remove('hidden')
    }
   },)
 console.log(user)
...
return (
    <>
      ...
      <div className="w-full max-w-md p-8 bg-white rounded-lg shadow-md">
        ...
        <p className="text-xs text-gray-500 mt-4" onClick={()=>{setDisplay(!display)}}>
        ...
        <p className="text-center text-sm text-indigo-500 mt-6 cursor-pointer">Forgot old Password?</p>
        <NavLink ref={passUpdateRef} className={"btn  text-center w-full py-2 my-2 mt-4 bg-teal-500 text-white rounded hover:bg-gray-800 hidden"} onClick={passwordUpdateEmail}>
