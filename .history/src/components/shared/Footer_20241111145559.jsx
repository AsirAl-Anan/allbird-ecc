    ...
    <footer className="bg-zinc-900 text-white">
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {regions.map((region) => (
          <button
            key={region.code}
            className="w-12 h-12 bg-gray-800 hover:bg-gray-700 transition-colors relative group flex items-center justify-center text-sm font-medium border border-gray-700"
          >
            {region.code}
            <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 text-xs bg-gray-800 text-white rounded opacity-0 group-hover:opacity-100 whitespace-nowrap transition-opacity border border-gray-700">
              {region.label}
            </span>
          </button>
        ))}
