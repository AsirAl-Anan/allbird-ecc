...
const Products = () => {
  ...
  const { cart, setCart ,addTocart } = useContext(AddCartContext)
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-[1500px] mx-auto px-4">
          {products.map((product) => (
            <ProductCard key={product.id} product={product} rating={renderStarRating} addTocart={addTocart} />
          ))}
        </div>
      </main>
      <Footer />
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 space-y-2">
        <NavLink onClick={()=>(addTocart(product))} className="w-full block bg-black text-white py-2 px-4 text-sm font-bold uppercase rounded transition-colors duration-300 ease-in-out hover:bg-gray-800">
          Add to Cart
        </NavLink>
        <NavLink  to={`product/${product.id}`} className="w-full block bg-white text-black border border-black py-2 px-4 text-sm font-bold uppercase rounded transition-colors duration-300 ease-in-out hover:bg-gray-100">
          View Product
        </NavLink>
      </div>
    </div>
