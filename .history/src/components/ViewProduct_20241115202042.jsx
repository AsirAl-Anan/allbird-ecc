...
const Recomendation = ({product ,currentProductId }) =>{
  ...
  useEffect( () => {
    ...
    try {
      ...
      setRecommendedProduct(
        data.filter((item)=> item.id !== currentProductId)
      )
   ...
   fetchData()
   function checkNotCommon (product) {
    return product.id !== currentProductId
  }
 setRecommendedProduct(recodata.filter(checkNotCommon))
        
    }
  }, [product, currentProductId, recodata]); // added product to dependency array //going on infinite loop

//  recodata.filter((e)=>console.log(currentProductId !== e.id))
